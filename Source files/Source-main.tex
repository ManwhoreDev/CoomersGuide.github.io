...
\documentclass[a4paper,12pt,american,oneside]{scrreprt} % KOMA-Script classes offer a ton of good features and are generally better for European users
...
factor=1000,
stretch=10,
shrink=10]{microtype} % Makes text look typograhically nicer
...
\usepackage[dvipsnames,cmyk,hyperref]{xcolor} % Adding custom colors
\usepackage{tagpdf} % Experimental Tagged PDF support for LaTeX
...
\usepackage[os=win]{menukeys} % Used for highlighting Input modes

\usepackage{subfiles} % For separating project into disctict files

\usepackage{moresize} % For typesetting cover
\usepackage{titling} % For typesetting cover

\usepackage{cleveref} % Easier labeling
...
\NewTColorBox[auto counter]{Inputbox}{ m }{% Input box
enhanced,
...
\tikz \node [label=right:Rerolled Response,draw,fill=red] (retry) {};
\end{center}
},  
}
